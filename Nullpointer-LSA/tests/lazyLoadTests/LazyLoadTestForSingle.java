...
class LazyLoadTestForSingle {
    ...
    void setUpTests() throws DatabaseException
    {
		try {
			...
			conn = dbm.getConnection(dbTables.SINGLE);
	...
	@Test
	void testsLazyLoadOnPowerToolSideForSingle() throws Exception {
		
		MapperManager.getSingleton().setMapperPackageName("mappersSingle");
		StripNailMapper sn = new StripNailMapper("111",11, 1,1,0.1);
	...
	@Test
	void testsLazyLoadOnStripNailSideForSingle() throws Exception 
	{
		MapperManager.getSingleton().setMapperPackageName("mappersSingle");
		StripNailMapper sn = new StripNailMapper("111",11, 1,1,0.1);
